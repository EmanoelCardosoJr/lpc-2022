...
while True:
    ...
    if ball.xcor() < -390:
        score_b += 1
        hud.clear()
        hud.write(f"{score_a} : {score_b}", align="center", font=("Courier", 24, "normal"))
    ...
    if (ball.xcor() > 340 and ball.xcor() < 350) and (((ball.ycor() >= paddle_b.ycor() + 25) and (ball.ycor() < paddle_b.ycor() + 50)) or ((ball.ycor() <= paddle_b.ycor() - 25) and (ball.ycor() > paddle_b.ycor() - 50))):
        ball.setx(340)
        ball.dx *= -1 # flip direction first
    ...
    if (ball.xcor() < -340 and ball.xcor() > -350) and (ball.ycor() < paddle_a.ycor() + 25 and ball.ycor() > paddle_a.ycor() - 25):
        ball.setx(-340)
        ball.dx *= -1

    if (ball.xcor() < -340 and ball.xcor() > -350) and (((ball.ycor() >= paddle_a.ycor() + 25) and (ball.ycor() < paddle_a.ycor() + 50)) or ((ball.ycor() <= paddle_a.ycor() - 25) and (ball.ycor() > paddle_a.ycor() - 50))):
        ball.setx(-340)
        ball.dx *= -1
        ball.dx += 0.1
